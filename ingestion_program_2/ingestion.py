...
if __name__ == "__main__":
    ...
    for seed_number in range(1, 4):
        ...
        torch.cuda.manual_seed_all(seed_number)  # if you are using multi-GPU.
        np.random.seed(seed_number)  # Numpy module.
        random.seed(seed_number)  # Python random module.
        torch.backends.cudnn.benchmark = False
        torch.backends.cudnn.deterministic = True

        run_output = os.path.join(output_dir, 'run_with_seed_' + str(seed_number))
        if not os.path.exists(run_output):
            os.makedirs(run_output)

        #=== Init a meta-learning environment
        env = Meta_Learning_Environment(train_data_dir, validation_data_dir, test_data_dir, meta_features_dir, algorithms_meta_features_dir, run_output, seed_number)
