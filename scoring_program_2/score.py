...
if __name__ == "__main__":
    ...
    ################## MAIN LOOP ##################
    number_of_runs = 3
    average_final_score = [0.0 for i in range(number_of_runs)]
    average_alc = [0.0 for i in range(number_of_runs)]
    list_dataset_alc = [None for i in range(number_of_runs)]
    for seed_number in range(1, 4):
        run_output_from_ingestion_program_dir = os.path.join(output_from_ingestion_program_dir, 'run_with_seed_' + str(seed_number))
        visualizations_dir = os.path.join(output_visualizations_dir, 'run_with_seed_' + str(seed_number))
        ...
        dataset_alc = pd.DataFrame(columns=['dataset', 'alc'], dtype=object)
        for dataset_name in list_datasets:
            ...
            try:
                output_file = os.path.join(run_output_from_ingestion_program_dir, dataset_name + '.csv')
            ...
            list_alc.append(alc)
            dataset_alc = dataset_alc.append({'dataset': dataset_name, 'alc':alc}, ignore_index=True)
        ...
        if len(list_final_score)!=0:
            average_final_score[seed_number-1] = round(sum(list_final_score) / len(list_datasets), 2)
        if len(list_alc)!=0:
            average_alc[seed_number-1] = round(sum(list_alc) / len(list_datasets), 2)

        #=== Write alc score of each dataset for this run
        dataset_alc = dataset_alc.append({'dataset': 'AVERAGE', 'alc':average_alc[seed_number-1]}, ignore_index=True)
        dataset_alc.to_csv(os.path.join(output_dir, 'RUN_WITH_SEED_' + str(seed_number) + '_RESULT_DATASET_ALC.csv'), index=False)
        list_dataset_alc[seed_number-1] = dataset_alc

    # Get best run
    worst_index = np.argmin(average_alc)

    #=== Write scores.html
    write_scores_html(output_dir, os.path.join(output_visualizations_dir, 'run_with_seed_' + str(worst_index)))

    #=== Write out the scores of the worst run to scores.txt
    score_file.write("average_final_score: " + str(average_final_score[worst_index]) + "\n")
    score_file.write("average_ALC: " + str(average_alc[worst_index]) + "\n")

    #=== Write alc scores of each dataset in the worst run
    list_dataset_alc[worst_index].to_csv(os.path.join(output_dir, 'RESULT_DATASET_ALC_WORST_RUN.csv'), index=False)

    vprint(verbose, "\n######################################## FINAL AVERAGE RESULTS ########################################")
    vprint(verbose, "\nlist_average_alc = " + str(average_alc))
    vprint(verbose, "\nlist_average_final_score = " + str(average_final_score))
    vprint(verbose, "\nworst_average_ALC = " + str(average_alc[worst_index]))
    vprint(verbose, "\ncorresponding_average_final_score = " + str(average_final_score[worst_index]))
