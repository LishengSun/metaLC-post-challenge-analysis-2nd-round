...
def bac_metric (solution, prediction, task='binary.classification'):
    ...
    score = (bac - base_bac) / np.maximum(eps, (1 - base_bac))
    return score
...
def f1_metric (solution, prediction, task='binary.classification'):
    ...
    # Bounding to avoid division by 0
    eps = 1e-15
    true_pos_num = np.maximum (eps, tp+fn)
    found_pos_num = np.maximum (eps, tp+fp)
    tp = np.maximum (eps, tp)
    tpr = tp / true_pos_num      # true positive rate (recall)
    ppv = tp / found_pos_num     # positive predictive value (precision)
    arithmetic_mean = 0.5 * np.maximum (eps, tpr+ppv)
    # Harmonic mean:
    ...
    # tpr=ppv=frac_pos, where frac_pos=1/label_num
        ...
        base_f1=1./label_num
    score = (f1 - base_f1) / np.maximum(eps, (1 - base_f1))
    return score
...
def log_loss(solution, prediction, task = 'binary.classification'):
    ...
    pred = np.minimum (1-eps, np.maximum (eps, pred))
...
def get_times(file_name, string_to_search):
    ...
    with open(file_name, 'r') as read_obj:
        ...
        for line in read_obj:
            ...
            temp = string_to_search.capitalize() + ' Training cycle'
            # print(temp)
            if temp in line:
                counter = 5
            counter = counter - 1
            if counter == 0:
                text = line.rstrip()
                time = re.findall("\d+\.\d+", text)
                list_of_results.append(float(time[0]))
...
#            'PAC (multiclass)':npac_multiclass_score}
    ...
    #---In AutoML, each dataset use its own metric
    dataset_metric = {'adult':'f1_binary_score', 'cadata':'r2_metric', 'digits':'nbac_multiclass_score', 'dorothea':'auc_metric', 'newsgroups':'npac_multiclass_score',
                        'christine':'nbac_binary_score', 'jasmine':'nbac_binary_score', 'madeline':'nbac_binary_score', 'philippine':'nbac_binary_score', 'sylvine':'nbac_binary_score',
                        'albert':'f1_binary_score', 'dilbert':'npac_multiclass_score', 'fabert':'npac_multiclass_score', 'robert':'nbac_multiclass_score', 'volkert':'npac_multiclass_score',
                        'alexis':'auc_metric', 'dionis':'nbac_multiclass_score', 'grigoris':'auc_metric', 'jannis':'nbac_multiclass_score', 'wallis':'auc_metric',
                        'evita':'auc_metric', 'flora':'a_metric', 'helena':'nbac_multiclass_score', 'tania':'npac_binary_score', 'yolanda':'r2_metric',
                        'arturo':'f1_multiclass_score', 'carlo':'npac_binary_score', 'marco':'auc_metric', 'pablo':'a_metric', 'waldo':'nbac_multiclass_score'
                        }

    #---Random Forests with different values of max_feature [0.1, ..., 1.0]
    algos = ['rf_mf_01', 'rf_mf_02', 'rf_mf_03', 'rf_mf_04', 'rf_mf_05', 'rf_mf_06',
            'rf_mf_07', 'rf_mf_08', 'rf_mf_09', 'rf_mf_10'
            ]

    solution_path = root + '/AllSolutions/'
    automl_results_path = root + '/automl_results/'
    learning_curves_output_path = root +  '/learning_curves/'


    # for type in ['valid', 'test']:
    #     for r in range(6):   #Loop through all 6 rounds
    #         print("\n#================================== ROUND " + str(r) + " ==================================#")
    #         path_round =  automl_results_path + "round_" + str(r)
    #         for a in os.scandir(path_round):
    #             if a.is_dir():
    #                 path_algo = a.path
    #                 algo_name = path_algo.split('/')[-1]
    #                 print("\n#=============== " + algo_name + " ===============#")
    #                 for dataset, metric in dataset_metric.items():
    #                     flag = False
    #                     if type=='test':
    #                         pattern = re.compile(dataset + "_test.*")
    #                     elif type=='valid':
    #                         pattern = re.compile(dataset + "_valid.*")
    #
    #                     for filepath in os.listdir(path_algo+'/output'):
    ...
    #                         try:
    #                             with open(solution_path + type + str(r) + '/' + dataset + '_' + type + '.solution') as f:
    #                                 sol = np.asarray([[float(digit) for digit in line.split()] for line in f])
    #                                 sol = sol.astype(np.float64)
    #                         except IOError:
    #                             continue
    #                         # print(path_algo)
    #                         times = get_times(path_algo + '/prediction_stdout_file.txt', dataset)
    #                         print("times=", times)
    #                         scores = []
    #                         for i in range(len(times)):
    #                             cycle_number = str(i).zfill(3)
    #                             with open(path_algo + '/output/' + dataset +'_' + type + '_' + cycle_number + '.predict') as f:
    #                                 pred = np.asarray([[float(digit) for digit in line.split()] for line in f])
    ...
    #
    #                         ## SAVE DATA
    #                         save_dir = learning_curves_output_path + type + '/' + dataset + '/' + algo_name + '/'
    #                         if not os.path.exists(save_dir):
    #                             os.makedirs(save_dir)
    #
    #                         with open(save_dir + "scores.txt", 'w') as writer:
    #                             writer.write('times: ' + str(times) + "\n")
    #                             writer.write('scores: ' + str(scores) + "\n")

                        # plt.plot(times, scores, label = "Dataset " + dataset)
                        #
                        # plt.xlabel('time (seconds)')
                        # plt.ylabel('performance metrics')
                        # plt.title('Learning curve of 01 algorithm on many datasets with different metrics')
                        # plt.legend(loc=4)
                        # plt.tight_layout()
                        # plt.show()
                        # plt.savefig("autoML_samples" + ".svg", dpi=1200, format='svg', bbox_inches='tight')

    for type in ['valid', 'test']:
        for it in os.scandir(learning_curves_output_path + type):
            if it.is_dir():
                path_dataset = it.path
                dataset_name = path_dataset.split('/')[-1]
                print(dataset_name)
                for algo_name in algos:
                    # if it.is_dir():
                        path_algo = path_dataset + '/' + algo_name
                        # algo_name = path_algo.split('/')[-1]
                        print(algo_name)
                        try:
                            with open(path_algo + "/scores.txt", "r") as data:
                                lines = data.readlines()
                                if len(lines)>1:
                                    dictionary = {line.split(":")[0]:line.split(":")[1].strip() for line in lines}
                                    scores = dictionary['scores']
                                    timestamps = dictionary['times']
                                    ...
                                    print(scores)
                                    plt.step(timestamps, scores, where='post', label = algo_name)
                                    plt.scatter(timestamps, scores, s=8, marker="D")
                        except FileNotFoundError:
                            print("File not found, return 0 as default!")
                plt.legend()
                plt.xlabel('time (seconds)')
                plt.ylabel('score')
                plt.title(label = "[" + type + "] - " + "Dataset: " + dataset_name + " - Metric: " + dataset_metric[dataset_name])
                # plt.show()
                plt.savefig(learning_curves_output_path + "visualizations/" + dataset_name + "_" + type + ".svg", dpi=1200, format='svg', bbox_inches='tight')
                plt.cla()
                plt.clf()
    ...
    # ### DIGITS ###
    # with open('/Volumes/DATA/CS/Freeze-Thaw/phase_0/digits/digits_test.solution') as f:
    #     sol = np.asarray([[float(digit) for digit in line.split()] for line in f])
    #     sol = sol.astype(np.float64)
    # with open('/Volumes/DATA/CS/Master-Thesis/output/digits_test_002.predict') as f:
    #     pred = np.asarray([[float(digit) for digit in line.split()] for line in f])
    #     pred = pred.astype(np.float64)
